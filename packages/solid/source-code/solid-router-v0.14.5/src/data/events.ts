...
export function setupNativeEvents(
  ...
  return (router: RouterContext) => {
    ...
    function handleAnchor(evt: MouseEvent) {
      ...
      const a = evt.composedPath().find(el => el instanceof Node && el.nodeName.toUpperCase() === 'A') as
        | HTMLAnchorElement
        | SVGAElement
        | undefined
    ...
    function handleFormSubmit(evt: SubmitEvent) {
      ...
      if (handler) {
        ...
        handler.call(
          ...
          (evt.target as HTMLFormElement).enctype === 'multipart/form-data' ? data : new URLSearchParams(data as any),
        )
      }
    }
