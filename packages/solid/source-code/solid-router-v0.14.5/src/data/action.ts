...
export type Action<T extends Array<any>, U> = (T extends [FormData] | [] ? JSX.SerializableAttributeValue : unknown) &
...
export function action<T extends Array<any>, U = void>(fn: (...args: T) => Promise<U>, name?: string): Action<T, U> {
...
function toAction<T extends Array<any>, U>(fn: Function, url: string): Action<T, U> {
  ...
  ;(fn as any).with = function <A extends any[], B extends any[]>(
    ...
    return toAction<B, U>(newFn as any, (uri.origin === 'https://action' ? uri.origin : '') + uri.pathname + uri.search)
  }
  ;(fn as any).url = url
  if (!isServer) {
