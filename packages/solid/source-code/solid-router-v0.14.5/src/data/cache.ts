import { createSignal, getListener, getOwner, onCleanup, sharedConfig, type Signal, startTransition } from 'solid-js'
...
export type CachedFunction<T extends (...args: any) => any> = T extends (...args: infer A) => infer R
  ? ([] extends { [K in keyof A]-?: A[K] } // A tuple full of optional values is equivalent to an empty tuple
      ? (...args: never[]) => R extends Promise<infer P> ? Promise<NarrowResponse<P>> : NarrowResponse<R>
...
export function cache<T extends (...args: any) => any>(fn: T, name: string): CachedFunction<T> {
  ...
  const cachedFn = ((...args: Parameters<T>) => {
    ...
    if (
      cached &&
      cached[0] &&
      (isServer || intent === 'native' || cached[3].count || Date.now() - cached[0] < PRELOAD_TIMEOUT)
    ) {
      if (tracking) {
        cached[3].count++
        cached[3][0]() // track
    ...
    if (cached) {
      ...
      cache.set(key, (cached = [now, res, intent, createSignal(now) as Signal<number> & { count: number }]))
      cached[3].count = 0
    }
    if (tracking) {
      cached[3].count++
...
cache.set = (key: string, value: any) => {
  ...
  if (cached) {
    ...
    cache.set(key, (cached = [now, value, , createSignal(now) as Signal<number> & { count: number }]))
    cached[3].count = 0
  }
}
